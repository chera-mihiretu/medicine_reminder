...
void main() {
  ...
  group("All Functions Should Work", () {
    ...
    test('should get a single medicine by id', () async {
      // arrange
      when(medicineRepository.getMedicine(any))
          .thenAnswer((_) async => Right(medicineEntity));
      // act
      final result =
          await medicineRepository.getMedicine(medicineEntity.medicineId);
      // assert
    ...
    test('should add a new medicine', () async {
      // arrange
      when(medicineRepository.addMedicine(any))
    ...
    test('should update an existing medicine', () async {
      // arrange
      when(medicineRepository.updateMedicine(any))
          .thenAnswer((_) async => const Right(true));
      // act
    ...
    test('should delete a medicine by id', () async {
      // arrange
      when(medicineRepository.deleteMedicine(any))
          .thenAnswer((_) async => const Right(true));
      // act
      final result =
          await medicineRepository.deleteMedicine(medicineEntity.medicineId);
      // assert
      expect(result, const Right(true));
  ...
  group("Failure Part Should also have to work", () {
    test('should return failure when getting list of medicines', () async {
      ...
      expect(
          result,
          equals(const Left<Failure, List<MedicineEntity>>(
              CacheFailure(message: ''))));
      verify(medicineRepository.getMedicines());
      verifyNoMoreInteractions(medicineRepository);
    });

    test('should return failure when getting a single medicine by id',
        () async {
      // arrange
      when(medicineRepository.getMedicine(any))
          .thenAnswer((_) async => const Left(CacheFailure(message: '')));
    ...
    test('should return failure when adding a new medicine', () async {
      ...
      // assert
      expect(
          result, equals(const Left<Failure, bool>(CacheFailure(message: ''))));
      verify(medicineRepository.addMedicine(medicineEntity));
      verifyNoMoreInteractions(medicineRepository);
    });
